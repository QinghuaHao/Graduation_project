...
def evaluate_model(model, test_loader, criterion):
    ...
    plt.savefig(f'../data/out_put_image/LSTM_model_Complex/{title}_confusion_matrix.png')
    plt.show()
...
def visualize_trajectory(data, labels, model, input_data, prediction, title):
    ...
    ax[2].legend()
    plt.savefig(f'../data/out_put_image/LSTM_model_Complex/{title}_training_progress.png')
    plt.show()
...
def visualize_training_progress(train_loss, train_accuracy, title):
    ...
    ax[1].legend()
    plt.savefig(f'../data/out_put_image/LSTM_model_Complex/{title}_trajectory.png')
    plt.show()
...
if __name__ == "__main__":
    ...
    }, 'Saved_Model/complex_motion_lstm_model.pth')

    # Load the trained model and predict new input trajectory
    checkpoint = torch.load('Saved_Model/complex_motion_lstm_model.pth')
